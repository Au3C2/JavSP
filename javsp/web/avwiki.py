...
def parse_data(movie: MovieInfo):
    ...
    title_tags = body.xpath("div/p/text()")
    if title_tags:
        title = title_tags[0]
    ...
    cite_links = body.xpath("//cite/a/@href")
    cite_url = cite_links[0] if cite_links else ''
    if cite_url:
        cite_url = cite_url.split('?aff=')[0]
    
    info_tags = body.xpath("dl[@class='dltable']")
    if info_tags:
        info = info_tags[0]
        ...
        for dt_txt, dd in zip(dt_txt_ls, dd_tags):
            ...
            a_tag = dd.xpath('a')
            if len(a_tag) == 0:
                dd_txt = dd.text.strip() if dd.text else ''
            else:
                dd_txt = [i.text.strip() for i in a_tag if i.text]
            if isinstance(dd_txt, list) and dt_txt != 'AV女優名':    # 只有女优名以列表的数据格式保留
                dd_txt = dd_txt[0] if dd_txt else ''
            data[dt_txt] = dd_txt
