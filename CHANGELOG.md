...
### Added
- **JavDB 交互式 CDP Cookie 提取**: 引入全新的 `javsp.cdp_cookie` 模块，利用底层 Chrome DevTools Protocol (CDP) 自动启动内置 Chromium/Edge 浏览器窗口，绕过 Cloudflare 防护及现代浏览器 App-Bound Encryption (`v20`) 磁盘加密限制。
- **自动抓取与持久化写回**: 当遭遇 Cloudflare 403 防护拦截时，会自动触发一次性弹窗过盾/登录，并将捕获的 Cookie 自动持久化保存到 `config.yml` 配置文件中。
...
### Fixed
- **CI 测试修复与鲁棒性优化**: 
  - 更新 `av-wiki` 封面比对基准数据，并在 CI 测试环境下优化图片路径校验策略（允许末尾文件名一致）。
  - 为工作流中 Public IP 外部检测步骤增加 `continue-on-error` 容错，避免因第三方接口网络波动中断测试。
  - 为工作流中的 `git fetch tags` 步骤添加 `--force` 参数以防止标签同步冲突。

### Optimized
- **标准进度条显示限制**: 为全项目 `tqdm` 进度条统一增加了 `ascii=True` 和 `ncols=120` 参数，避免不同终端字符集及窗口宽度下的显示错乱问题。
